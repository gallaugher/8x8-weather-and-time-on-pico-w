# code.py — Weather display for 32×8 NeoPixel matrix
# Location name is card coded as CITY_NAME. Be sure to update.
import board
import time
import neopixel
import wifi
...
def scroll_text(text, color=0x00FF00, speed=0.01):
    print(f"Scrolling: {text} in color {hex(color)}")
    text_width = len(text) * 6
    for scroll_pos in range(text_width + WIDTH):
        pixel_framebuf.fill(0x000000)
        pixel_framebuf.text(text, WIDTH - scroll_pos, 0, color)
        pixel_framebuf.display()
        # time.sleep(speed) # I stopped sleep - it seems there was enough delay in just the running of this code.
